...
user_health=$(test_endpoint "User Service" "http://localhost:3000/api/users/health")
...
register_response=$(test_endpoint "User Service - Register" "http://localhost:3000/api/users/register" "POST" "$register_data")

# Extraer token de autenticación
echo "Iniciando sesión con usuario de prueba..." | tee -a $LOG_FILE
login_data='{"email":"test@example.com","password":"Password123!"}'
login_response=$(test_endpoint "User Service - Login" "http://localhost:3000/api/users/login" "POST" "$login_data")
...
message_health=$(test_endpoint "Message Service" "http://localhost:3000/api/messages/health")
...
create_message=$(test_endpoint "Message Service - Create" "http://localhost:3000/api/messages" "POST" "$message_data" "$token")

# Probar File Service
echo "=== Probando File Service ===" | tee -a $LOG_FILE
file_health=$(test_endpoint "File Service" "http://localhost:3000/api/files/health")

# Probar Task Service
echo "=== Probando Task Service ===" | tee -a $LOG_FILE
task_health=$(test_endpoint "Task Service" "http://localhost:3000/api/tasks/health")
...
create_task=$(test_endpoint "Task Service - Create" "http://localhost:3000/api/tasks" "POST" "$task_data" "$token")
...
if [ ! -z "$task_id" ] && [ "$task_id" != "null" ]; then
    ...
    dependencies_health=$(test_endpoint "Dependencies Service" "http://localhost:3000/api/dependencies/health")
    ...
    create_dependency=$(test_endpoint "Dependencies Service - Create" "http://localhost:3000/api/dependencies" "POST" "$dependency_data" "$token")
fi

# Probar Timeline Service
echo "=== Probando Timeline Service ===" | tee -a $LOG_FILE
timeline_health=$(test_endpoint "Timeline Service" "http://localhost:3000/api/timeline/health")
...
create_event=$(test_endpoint "Timeline Service - Create Event" "http://localhost:3000/api/timeline/events" "POST" "$event_data" "$token")
